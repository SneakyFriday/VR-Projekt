...
public class GästeSpawner : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        Debug.Log(waypoints.Length+" Waypoint length");
    ...
    { 
        ...
        {
            Instantiate(whatToSpawnPrefab, spawnPoint.transform.position, Quaternion.identity); // Spawnt das Prefab, an einem random Platz. 
            seatTaken[seatNumber] = true; // Der Sitzplatz wird als besetzt markiert.
            //Debug.Log("Gast an Tisch "+ seatNumber+" Eingetroffen");
            
        }
        //return waypoints[seatNumber]; // Gibt die Nummer des Sitzplatzes zurück.
        seat = waypoints[seatNumber];
        Debug.Log(seat.transform+"- Seat in SpawnObjekt ");
    } 
